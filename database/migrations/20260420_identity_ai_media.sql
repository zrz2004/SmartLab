...
CREATE TABLE IF NOT EXISTS inspection_media_refs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  inspection_id UUID NOT NULL REFERENCES inspection_records(id) ON DELETE CASCADE,
  nocodb_record_id VARCHAR(128) NOT NULL,
...
ALTER TABLE alerts
  ADD COLUMN IF NOT EXISTS source VARCHAR(32) NOT NULL DEFAULT 'sensor',
  ADD COLUMN IF NOT EXISTS inspection_id UUID REFERENCES inspection_records(id),
